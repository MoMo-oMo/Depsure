  ...
  <div class="view-agency-page">
    <v-container fluid>
      ...
      <v-row class="mb-4">
        ...
        <v-col
          v-if="false && isAgencyUser"
          class="pa-4 d-flex align-center"
          cols="12"
          lg="2"
          md="2"
        >
          <v-btn class="back-btn" @click="addNotice"> Add Notice </v-btn>
        </v-col>
      </v-row>
      ...
      <v-row>
        <v-col class="pa-4" cols="12">
          ...
          >
            ...
            <template #item.monthsMissedRent="{ item }">
    ...
    data () {
      return {
        ...
        headers: [
          ...
          {
            title: 'MONTH\'S MISSED RENT',
            key: 'monthsMissedRent',
            sortable: true,
            align: 'center',
          },
    ...
    methods: {
      ...
      filterProperties () {
        this.filteredProperties = this.properties.filter(property => {
          const textMatch = property.unitName
            .toLowerCase()
      ...
      async fetchNotices (agencyId = null) {
        ...
        try {
          ...
          const noticesWithPropertyTypes = await Promise.all(
            notices.map(async notice => {
              try {
                ...
                console.error(
                  'Error resolving property type for notice:',
                  notice.id,
                  error,
                )
                return { ...notice, propertyType: 'residential' }
          ...
          this.properties = noticesWithPropertyTypes.filter(
      ...
      async moveToVacancies (notice) {
        ...
        try {
          ...
          try {
            ...
            console.warn('Failed to refresh latest notice before processing', error)
          }
          ...
          if (!unitDoc) {
            ...
            unitDoc = noticeTaggedSnapshot.empty
              ? null
              : noticeTaggedSnapshot.docs[0]
          }
          ...
          for (const { field, value } of vacancyChecks) {
            if (!value) continue
            const snapshot = await getDocs(
              query(vacancyCollectionRef, where(field, '==', value)),
            )
            if (!snapshot.empty) {
              this.showErrorDialog(
          ...
          this.showSuccessDialog(
            `Vacancy created for unit "${displayUnitName}". Notice has been deleted. Unit remains in active units.`,
            'Success!',
            'Continue',
          )
        } catch (error) {
          ...
          this.showErrorDialog(
            `Failed to process notice: ${error.message}`,
            'Error',
            'OK',
          )
        }
      ...
      async deleteProperty (property) {
        ...
        try {
          ...
          this.showSuccessDialog(
            `Notice for ${property.unitName} deleted successfully!`,
            'Success!',
            'Continue',
          )
        } catch (error) {
