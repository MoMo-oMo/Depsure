<template>
  <div ref="pageRoot" class="messenger-page">
    <!-- Header -->
    <div class="header">
      <div class="header-gradient">
        <div class="header-row">
          ...
          <div class="header-user">
            <v-avatar class="avatar-ring" size="44">
              <img alt="Avatar" :src="chatAvatarUrl || fallbackAvatar">
            </v-avatar>
            <div class="user-meta">
              <div class="name">
              ...
              <div class="status">
                <span
                  class="dot"
                  :class="isPartnerOnline ? 'online' : 'offline'"
                />
                {{ statusText }}
              </div>
            </div>
    ...
    <div ref="chat" class="chat-scroll">
      <template v-for="(m, idx) in normalizedMessages" :key="m._key">
        ...
        <div v-if="shouldShowDate(idx)" class="time-chip">
          {{ dateChipLabel(m) }}
        </div>
        ...
        <div
          ...
          :class="[
            m.direction,
            {
              selecting: selectionMode && canEdit(m),
              'drag-over': dragOverId === m.id,
            },
          ]"
          :style="rowStyle(m)"
        ...
        >
          ...
          <div
            v-if="m.deleted"
            class="bubble deleted-bubble"
            :class="m.direction"
          >
            <span class="deleted-icon"><v-icon
              color="#94a3b8"
              size="14"
            >mdi-clock-outline</v-icon></span>
          ...
          >
            ...
            <img
              ...
              class="message-image"
              :src="m.imageUrl"
              @click.stop="openImagePreview(m.imageUrl)"
            >
            <div class="bubble-meta">
              ...
              <div v-if="m.direction === 'outgoing'" class="message-status">
                ...
                <div
                  v-else-if="getMessageStatus(m) === 'delivered'"
                  class="double-ticks delivered"
                >
                  <v-icon
                    color="rgba(255,255,255,0.6)"
                    size="12"
                ...
                <div
                  v-else-if="getMessageStatus(m) === 'read'"
                  class="double-ticks read"
                >
                  <v-icon color="#4fc3f7" size="12">mdi-check</v-icon>
                  <v-icon color="#4fc3f7" size="12">mdi-check</v-icon>
                </div>
              </div>
            </div>
            <div
              v-if="m.reaction"
              class="reaction-under"
              :class="m.direction"
              title="Remove reaction"
              @click="clearReaction(m)"
            >
              {{ m.reaction }}
            </div>
          </div>
          ...
          <div v-if="false" class="inline-time">{{ m.time }}</div>
        </div>
      </template>
    ...
    <!-- Composer -->
    <div ref="composer" class="composer my-6">
      <div class="composer-inner">
        ...
        <!-- Send -->
        <button aria-label="Send" class="send" @click="handleSend">
    ...
    <div
      v-if="reactionPicker && reactionPicker.show"
      ref="reactionPanel"
    ...
    >
      <button
        v-for="e in safeReactionEmojis"
        :key="e"
        class="reaction-btn"
        @click.stop="selectReaction(e)"
      >
        {{ e }}
      </button>
    </div>
    ...
    >
      <img alt="image" class="img-full" :src="imagePreview.src">
    </div>

    <!-- Header actions menu -->
    <div
      v-if="headerMenu && headerMenu.show"
      class="msg-menu"
    ...
    computed: {
        ...
        if (this.isAgencyUser) {
          ...
          try {
            ...
            for (let i = arr.length - 1; i >= 0; i--) {
              ...
              if (
                m
                && String(m.authorId || '') !== String(this.currentUserId || '')
                && m.authorAvatarUrl) return m.authorAvatarUrl
            }
          } catch {}
          ...
          if (this.superAdminProfile) {
            return (
              this.superAdminProfile.profileImageUrl
              || this.superAdminProfile.profileImage
              || 'https://images.pexels.com/photos/186077/pexels-photo-186077.jpeg'
            )
          }
      ...
      isPartnerOnline () {
        try {
          ...
          const thresholdMs = 120_000 // 2 minutes
          const heuristicOnline
            = !!lastIncoming && Date.now() - lastIncoming < thresholdMs
      ...
      isOverlayActive () {
        return !!(
          ...
          || (this.headerMenu && this.headerMenu.show)
          || (this.reactionPicker && this.reactionPicker.show)
        )
      },
      ...
      canBulkDelete () {
        ...
        return (
          arr.length > 0
          && arr.every(m => m.direction === 'outgoing' && !m.deleted)
        )
      },
      singleSelected () {
    ...
    async mounted () {
      ...
      if (this.isAgencyUser) {
        ...
        this.selectedAgencyId
          = user.userType === 'Agency' ? user.uid : user.managedAgencyId
        await this.loadChat()
      } else {
    ...
    methods: {
          ...
          if (
            ...
            if (user.userType === 'Agency') {
              ...
              const agencyDoc = await getDoc(
                doc(db, 'users', user.managedAgencyId),
              )
              if (agencyDoc.exists())
                agencyData = { id: agencyDoc.id, ...agencyDoc.data() }
            }
        ...
        try {
          ...
          this.unsubscribe = onSnapshot(
            ...
            error => {
              ...
              this.showErrorDialog
                && this.showErrorDialog(
                  `Chat subscription failed: ${error.message}`,
                  'Error',
                  'OK',
                )
            },
      ...
      async handleSend () {
        ...
        try {
          ...
          if (user?.userType === 'Agency') {
            // Agency users: prefer person's name if available, else fall back to agency name
            authorName
            ...
            user?.userType === 'Admin'
            && user?.adminScope === 'agency'
          ) {
            // Agency Admin users: use person's name, not agency name
            authorName
              = user?.firstName && user?.lastName
                ? `${user.firstName} ${user.lastName}`
                : user?.firstName
            ...
            // For Super Admin and Depsure Admin users, use their name or email
          ...
          const message = {
            ...
            ...(this.replyTo
              ? { replyTo: this.buildReplyRef(this.replyTo) }
              : {}),
          }

          await setDoc(
            doc(db, 'chats', this.chatDocId, 'messages', String(message.id)),
            message,
          )
          try {
            await updateDoc(doc(db, 'chats', this.chatDocId), {
      ...
      async deleteSelected () {
        ...
        if (toDelete.length === 0 || !this.chatDocId) return
        try {
          ...
          for (const mid of ids) {
            ...
            if (msnap.exists()) {
              ...
              if (String(data.authorId || '') === userId && !data.deleted) {
                batchOps.push(
                  updateDoc(msgDoc, { deleted: true, deletedAt: new Date() }),
                )
        ...
        try {
          ...
          if (typeof m === 'object') {
            ...
            if (m.type && m.type !== 'text') {
              ...
              if (m.type === 'pdf')
                return m.fileName ? `PDF · ${m.fileName}` : 'PDF document'
              if (m.type === 'file')
      ...
      getSenderColor (senderName) {
        ...
        for (let i = 0; i < senderName.length; i++) {
          hash = ((hash << 5) - hash + senderName.charCodeAt(i)) & 0xff_ff_ff_ff
        }
      ...
      getOtherUserId () {
        ...
        if (
          this.selectedAgencyId
          && this.selectedAgencyId !== this.currentUserId
        ) {
          return this.selectedAgencyId
        }

        // For agency chats, find any user who is not the current user
        const otherUser = this.messages.find(
          m => m.authorId !== this.currentUserId,
        )
      ...
      async markMessagesAsRead () {
        ...
        try {
          ...
          for (const m of this.messages || []) {
            ...
            ops.push(
              updateDoc(msgDoc, { readBy: arrayUnion(this.currentUserId) }),
            )
          }
          if (ops.length > 0) {
      ...
      async clearUnreadCounter () {
        try {
          ...
          await setDoc(
            doc(db, 'unread', uid, 'chats', this.chatDocId),
            { count: 0, updatedAt: serverTimestamp() },
            { merge: true },
          )
        } catch {}
      },
      ...
      isSelected (m) {
        return !!(m && this.selected[String(m.id)])
      },
      toggleSelect (m) {
        if (!m) return
        ...
        if (Object.values(this.selected).every(v => !v))
          this.selectionMode = false
      },
      rowStyle (m) {
        ...
        const tr
          = m && typeof m._offsetX === 'number'
            ? `translateX(${m._offsetX}px)`
            : undefined
        return {
          position: 'relative',
      ...
      async onDropOnMessage (m, ev) {
        try {
          const emoji
            = ev.dataTransfer?.getData('application/x-emoji')
      ...
      async clearReaction (m) {
        ...
        try {
          ...
          console.error('Clear reaction failed', error)
        }
      },
      ...
      updateSafeGaps () {
        try {
          ...
          try {
            const cs
              = comp && window.getComputedStyle
                ? window.getComputedStyle(comp)
                : null
            composerMarginBottom = cs ? Number.parseFloat(cs.marginBottom || '0') : 0
          ...
          const safeGapPx = Math.round(
            composerHeight + composerMarginBottom + panelExtra + safetyPad,
          )
      ...
      isPresenceOnline (p) {
        try {
          ...
          return Date.now() - d.getTime() < 90_000
        } catch {
          return false
        }
      },
      ...
      getEmojiOptions () {
        ...
        if (this.emojiCategory === 'recent' && this.recentEmojis.length > 0)
      ...
      insertEmoji (e) {
        ...
        if (!inserted) {
          el.append(document.createTextNode(e))
        }
        ...
        if (!inserted) {
          setTimeout(() => {
            try {
              ...
              else el.append(document.createTextNode(e))
      ...
      async onFilesSelected (ev) {
        try {
          ...
          ev.target.value = ''
          if (files.length === 0 || !this.chatDocId) return
...
.messenger-page::before {
  ...
  background: radial-gradient(
      circle at 10% 10%,
      rgba(152, 201, 232, 0.08),
      transparent 40%
    ),
    radial-gradient(
      circle at 90% 20%,
      rgba(111, 176, 43, 0.07),
      transparent 40%
    ),
    radial-gradient(
      circle at 50% 80%,
      rgba(26, 151, 212, 0.06),
      transparent 40%
    );
  opacity: 1;
  pointer-events: none;
...
.image-bubble.incoming .message-image {
  border-radius: 18px;
}
.image-bubble .bubble-meta {
  position: absolute;
  right: 10px;
  bottom: 10px;
}
.image-bubble .bubble-meta .time {
  color: #ffffff;
  text-shadow: 0 1px 2px rgba(0, 0, 0, 0.6);
}
.image-bubble.incoming .bubble-meta .time {
  color: #64748b;
  text-shadow: none;
}
...
.text-bubble.incoming .bubble-meta .time {
  color: #64748b;
}
.text-bubble.incoming .edited-inline {
  color: #64748b;
}
.text-bubble.outgoing .bubble-meta .time {
  color: rgba(255, 255, 255, 0.9);
}
...
.reaction-under.incoming {
  left: 6px;
}
.reaction-under.outgoing {
  right: 10px;
}
