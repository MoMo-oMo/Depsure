...
async function getUserTokens(userIds = []) {
  ...
  userDocs.forEach(snap => {
    ...
    arr.forEach(t => tokens.add(String(t)));
  });
  ...
  await Promise.all(userIds.map(async (uid) => {
    ...
    snapshot.forEach(doc => tokens.add(String(doc.id)));
  }));
...
exports.onChatUpdatedSendPush = onDocumentUpdated('chats/{chatId}', async (event) => {
  try {
    ...
    const last = Array.isArray(after.messages) ? after.messages[after.messages.length - 1] : null;
    if (!last || !last.text || !last.authorId) return;
    ...
    staffSnap.forEach(doc => {
      ...
      if (isDepsure && doc.id !== last.authorId) recipients.add(doc.id);
    });
    ...
    if (agencyId) {
      ...
      try {
        ...
        logger.warn('Agency admins query failed; falling back to all admins (may over-notify)', { error: String(e) });
        staffSnap.forEach(doc => {
          const u = doc.data() || {};
    ...
    logger.error('onChatUpdatedSendPush error', { error: err });
  }
});
...
exports.onChatMessageCreatedSendPush = onDocumentCreated('chats/{chatId}/messages/{messageId}', async (event) => {
  try {
    ...
    staffSnap.forEach(doc => {
      const u = doc.data() || {}
      const isDepsure = u.userType === 'Super Admin' || (u.userType === 'Admin' && String(u.adminScope || '') === 'depsure')
      if (isDepsure && doc.id !== msg.authorId) recipients.add(doc.id)
    })
    if (agencyId) {
    ...
    try {
      ...
      logger.warn('Failed to update unread counters', { error: String(e) })
    }
  } catch (err) {
    logger.error('onChatMessageCreatedSendPush error', { error: err })
  }
})
